...
export default function App() {
    ...
    const { messages, isSending, uploadProgress, sendText, sendPhoto, acceptPhoto } = useChat(
        user,
        nickname
    );
    ...
    const handlePhotoConfirm = async ({
        ...
        try {
            await sendPhoto({ file, url }, pixelSize, (p) => {
                // optional local progress callback
            });
    ...
    return (
        ...
        {status === "splash" && (
            ...
            <button
            className="btn-main mt-4 px-4 py-2 bg-primary rounded"
